...
# Set page config
st.set_page_config(
    page_title="Defense Tender Analyzer",
    page_icon="⚡",
    layout="wide"
)
...
class OptimizedDefenseTenderLLM:
    ...
    def load_models(_self):
        ...
        try:
            with st.spinner("⚡ Loading optimized models... (45 seconds)"):
                models = {}
                ...
                try:
                    ...
                    st.success("✅ Fast text generation ready!")
                ...
                try:
                    ...
                    st.success("✅ Fast QA system ready!")
                except Exception as e:
                ...
                try:
                    ...
                    st.success("✅ Fast embeddings ready!")
                except Exception as e:
                ...
                st.success("🚀 All optimized models loaded!")
                return models
        ...
        try:
            ...
            def process_single_file(file_info):
                ...
                try:
                    ...
                    st.warning(f"⚠️ Error with {uploaded_file.name}: {e}")
            ...
            for idx, uploaded_file in enumerate(uploaded_files):
                status_text.text(f"⚡ Fast processing: {uploaded_file.name}")
                ...
                st.success(f"✅ {uploaded_file.name}: {len(file_docs)} chunks")
            
            if self.documents:
                # Fast embedding creation
                self.create_embeddings_fast(models['embedding_model'])
                st.success(f"🚀 Processed {total_docs} chunks from {len(uploaded_files)} files!")
                return True
            else:
    ...
    def create_embeddings_fast(self, embedding_model):
        ...
        st.success(f"⚡ Embeddings ready in {embed_time:.1f}s!")
...
def main():
    ...
    st.title("⚡ Fast & Accurate Defense Tender Analyzer")
    st.markdown("**Optimized for Speed & Accuracy**")
    
    # Speed metrics
    col1, col2, col3, col4 = st.columns(4)
    with col1:
        st.metric("⚡ Processing", "Ultra Fast")
    with col2:
        st.metric("🧠 Models", "Optimized")
    with col3:
        st.metric("⏱️ Load Time", "45 sec")
    with col4:
    ...
    if st.session_state.models is None:
        ...
        if st.session_state.models is None:
            st.error("❌ Model loading failed. Please refresh.")
            st.stop()
    ...
    with st.sidebar:
        ...
        if uploaded_files:
            ...
            for file in uploaded_files:
                ...
                total_size += size_mb
                file_info.append(f"📄 {file.name} ({size_mb:.1f} MB)")
            
            st.info(f"**{len(uploaded_files)} files** ({total_size:.1f} MB total)")
            for info in file_info:
                st.text(info)
            
            if st.button("⚡ Fast Process", type="primary"):
                start_time = time.time()
        ...
        if st.session_state.documents_processed:
            ...
            st.metric("⏱️ Process Time", f"{stats.get('time', 0):.1f}s")
    ...
    with col1:
        st.header("💬 Lightning Fast Q&A")
        
        if not st.session_state.documents_processed:
            st.info("👈 Upload documents for instant analysis!")
            
            with st.expander("⚡ Speed Optimizations"):
                st.markdown("""
            ...
            if st.button("⚡ Get Instant Answer", type="primary") and question.strip():
                ...
                with metric_row[0]:
                    st.metric("⚡ Speed", f"{answer_time:.1f}s")
                with metric_row[1]:
                    st.metric("🎯 Confidence", f"{result['confidence']:.0%}")
                with metric_row[2]:
                    method_clean = result['method'].replace('_', ' ').title()
                    st.metric("🔧 Method", method_clean)
                with metric_row[3]:
